/**
 * IdCtaCteType.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis 1.4 Apr 22, 2006 (06:55:48 PDT) WSDL2Java emitter.
 */

package coop.guenoa.afip.FECredService;

public class IdCtaCteType implements java.io.Serializable {
	private java.lang.Long codCtaCte;

	private coop.guenoa.afip.FECredService.IdComprobanteType idFactura;

	public IdCtaCteType() {
	}

	public IdCtaCteType(java.lang.Long codCtaCte, coop.guenoa.afip.FECredService.IdComprobanteType idFactura) {
		this.codCtaCte = codCtaCte;
		this.idFactura = idFactura;
	}

	/**
	 * Gets the codCtaCte value for this IdCtaCteType.
	 * 
	 * @return codCtaCte
	 */
	public java.lang.Long getCodCtaCte() {
		return codCtaCte;
	}

	/**
	 * Sets the codCtaCte value for this IdCtaCteType.
	 * 
	 * @param codCtaCte
	 */
	public void setCodCtaCte(java.lang.Long codCtaCte) {
		this.codCtaCte = codCtaCte;
	}

	/**
	 * Gets the idFactura value for this IdCtaCteType.
	 * 
	 * @return idFactura
	 */
	public coop.guenoa.afip.FECredService.IdComprobanteType getIdFactura() {
		return idFactura;
	}

	/**
	 * Sets the idFactura value for this IdCtaCteType.
	 * 
	 * @param idFactura
	 */
	public void setIdFactura(coop.guenoa.afip.FECredService.IdComprobanteType idFactura) {
		this.idFactura = idFactura;
	}

	private java.lang.Object __equalsCalc = null;

	public synchronized boolean equals(java.lang.Object obj) {
		if (!(obj instanceof IdCtaCteType))
			return false;
		IdCtaCteType other = (IdCtaCteType) obj;
		if (obj == null)
			return false;
		if (this == obj)
			return true;
		if (__equalsCalc != null) {
			return (__equalsCalc == obj);
		}
		__equalsCalc = obj;
		boolean _equals;
		_equals = true
				&& ((this.codCtaCte == null && other.getCodCtaCte() == null)
						|| (this.codCtaCte != null && this.codCtaCte.equals(other.getCodCtaCte())))
				&& ((this.idFactura == null && other.getIdFactura() == null)
						|| (this.idFactura != null && this.idFactura.equals(other.getIdFactura())));
		__equalsCalc = null;
		return _equals;
	}

	private boolean __hashCodeCalc = false;

	public synchronized int hashCode() {
		if (__hashCodeCalc) {
			return 0;
		}
		__hashCodeCalc = true;
		int _hashCode = 1;
		if (getCodCtaCte() != null) {
			_hashCode += getCodCtaCte().hashCode();
		}
		if (getIdFactura() != null) {
			_hashCode += getIdFactura().hashCode();
		}
		__hashCodeCalc = false;
		return _hashCode;
	}

	// Type metadata
	private static org.apache.axis.description.TypeDesc typeDesc = new org.apache.axis.description.TypeDesc(
			IdCtaCteType.class, true);

	static {
		typeDesc.setXmlType(
				new javax.xml.namespace.QName("http://ar.gob.afip.wsfecred/FECredService/", "IdCtaCteType"));
		org.apache.axis.description.ElementDesc elemField = new org.apache.axis.description.ElementDesc();
		elemField.setFieldName("codCtaCte");
		elemField.setXmlName(new javax.xml.namespace.QName("", "codCtaCte"));
		elemField.setXmlType(new javax.xml.namespace.QName("http://www.w3.org/2001/XMLSchema", "long"));
		elemField.setMinOccurs(0);
		elemField.setNillable(false);
		typeDesc.addFieldDesc(elemField);
		elemField = new org.apache.axis.description.ElementDesc();
		elemField.setFieldName("idFactura");
		elemField.setXmlName(new javax.xml.namespace.QName("", "idFactura"));
		elemField.setXmlType(
				new javax.xml.namespace.QName("http://ar.gob.afip.wsfecred/FECredService/", "IdComprobanteType"));
		elemField.setMinOccurs(0);
		elemField.setNillable(false);
		typeDesc.addFieldDesc(elemField);
	}

	/**
	 * Return type metadata object
	 */
	public static org.apache.axis.description.TypeDesc getTypeDesc() {
		return typeDesc;
	}

	/**
	 * Get Custom Serializer
	 */
	public static org.apache.axis.encoding.Serializer getSerializer(java.lang.String mechType,
			java.lang.Class _javaType, javax.xml.namespace.QName _xmlType) {
		return new org.apache.axis.encoding.ser.BeanSerializer(_javaType, _xmlType, typeDesc);
	}

	/**
	 * Get Custom Deserializer
	 */
	public static org.apache.axis.encoding.Deserializer getDeserializer(java.lang.String mechType,
			java.lang.Class _javaType, javax.xml.namespace.QName _xmlType) {
		return new org.apache.axis.encoding.ser.BeanDeserializer(_javaType, _xmlType, typeDesc);
	}

}
